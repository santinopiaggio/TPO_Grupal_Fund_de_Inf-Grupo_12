import random
def op1(lst_pilotos,lst_numeros,lst_escuderias,lst_puntos,lst_tiempos,lst_presupuestos,lst_abandonos,split):
...
# Eliminar piloto
def op2(lst_numeros, lst_puntos, lst_escuderias, lst_pilotos, lst_tiempos, lst_presupuestos, lst_abandonos,puntos):
    indice = 0
    numero_monoplaza = input("Ingrese el numero del piloto que desea eliminar: ")
    
    while numero_monoplaza not in lst_numeros:
        numero_monoplaza = input("ERROR. Ingrese el numero del piloto que desea eliminar: ")
    
    if numero_monoplaza in numero_monoplaza and puntos == 0:
...
def op3(lst_puntos,lst_promedios,lst_nombres,lst_numeros):
    modificar = input("Nombre del piloto o numero del monoplaza: ")
    while modificar not in lst_nombres or modificar not in lst_numeros:
    ...
    if decision == 1:
        if modificar in lst_numeros:
            posicion = lst_numeros.index(modificar)
            puntos_mod = int(input("Ingrese la cantidad de puntos a agregar o quitar (valor positivo o negativo): "))
            lst_puntos[posicion] = lst_puntos [posicion] + puntos_mod


            posicion = lst_nombres.index(modificar)
            puntos_mod = int(input("Ingrese la cantidad de puntos a agregar o quitar (valor positivo o negativo): "))
            lst_puntos[posicion] = lst_puntos [posicion] + puntos_mod
...
def main ():
    ...
    while opcion != 5:
        while opcion == 1:
            op1(lst_pilotos, lst_numeros,lst_escuderias,lst_puntos,lst_tiempos,lst_presupuestos,lst_abandonos)
        while opcion == 2:
            op2(lst_pilotos, lst_numeros,lst_escuderias,lst_puntos,lst_tiempos,lst_presupuestos,lst_abandonos)
        while opcion == 3:
            op3(lst_pilotos, lst_numeros,lst_escuderias,lst_puntos,lst_tiempos,lst_presupuestos,lst_abandonos)    
lst_pilotos = ['norris','colapinto','hamilton']
lst_numeros = [16,2,321]
lst_escuderias = ['ferrari','alpine','mclaren']
lst_puntos = [26,10,30]
lst_tiempos = [87.90,89.15,90]
...
def op2(lst_numeros, lst_puntos, lst_escuderias, lst_pilotos, lst_promedios, lst_presupuestos, lst_abandonos):
    indice = 0
    numero_monoplaza = int(input("ERROR. Ingrese el numero del piloto que desea eliminar: "))
    conf = False
    
    while numero_monoplaza not in lst_numeros:
        numero_monoplaza = int(input("ERROR. Ingrese el numero del piloto que desea eliminar: "))
    
    if numero_monoplaza in lst_numeros and lst_puntos == 0:
...
def op3(lst_pilotos,lst_numeros,lst_puntos,lst_promedios):
    modificar = input("Nombre del piloto o numero del monoplaza: ")
    lst_numeros=str(lst_numeros)
    while modificar not in lst_pilotos and modificar not in lst_numeros:
        modificar = input("No se encuentra el dato ingresado. Ingrese un nombre o numero valido: ")
    ...
    if decision == 1:
        if modificar in lst_numeros:
            ...
            print('el piloto tiene ',lst_puntos[posicion])
            main()
        else:
        ...
        if modificar in lst_numeros:
            posicion = lst_numeros.index(modificar)
            tiempo_mod = float(input("Nuevo tiempo promedio: "))
            while tiempo_mod<0:
                tiempo_mod=float(input('ingrese un numero positivo'))
            lst_promedios[posicion] = tiempo_mod
            main()
        else:
            posicion = lst_pilotos.index(modificar)
